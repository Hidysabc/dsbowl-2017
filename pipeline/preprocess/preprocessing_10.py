...
def main():
    ...
    while(len(patient_id))>0:
        for i in np.arange(len(patient_id[:args.count])):
            patient_imgs_keys = [j for j in all_keys if patient_id[i] in j]
            ...
            logger.debug("Successfully downloaded images from S3 patient_id:{}!".format(patient_id[i]))
            '''
        ...
        #patients_spacing = []
        #output = []
        for i in np.arange(len(patients[:args.count])):
